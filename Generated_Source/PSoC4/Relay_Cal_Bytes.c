...
/*******************************************************************************
* Function Name: Relay_Cal_Bytes_Start
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Relay_Cal_Bytes_Stop
********************************************************************************
*
* Summary:
*  Empty function. Included for consistency with other
...
/*******************************************************************************
* Function Name: Relay_Cal_Bytes_Write
********************************************************************************
*
* Summary:
...
*******************************************************************************/
#if (CY_PSOC3)
    cystatus Relay_Cal_Bytes_Write(const uint8 srcBuf[], const uint8 eepromPtr[], uint16 byteCount)
...
#endif /* (!CY_PSOC3) */
{
    ...
    #if (CY_PSOC3)
        eeOffset &= Relay_Cal_Bytes_CODE_ADDR_MASK;
    #endif /* (CY_PSOC3) */

    /* Make sure, that eepromPtr[] points to ROM */
    #if (CY_PSOC3)
        if (((uint32)eepromPtr >= Relay_Cal_Bytes_CODE_ADDR_OFFSET) &&
            (((uint32)eepromPtr + byteCount) <= Relay_Cal_Bytes_CODE_ADDR_END))
    #else
        if (((uint32)eepromPtr + byteCount) < Relay_Cal_Bytes_FLASH_END_ADDR)
    #endif /* (CY_PSOC3) */
    {
        #if (!CY_PSOC4)
            (void)CySetTemp();
        ...
        rowId = (eeOffset / CY_FLASH_SIZEOF_ROW) % Relay_Cal_Bytes_ROWS_IN_ARRAY;
        ...
        while ((srcIndex < byteCount) && (CYRET_SUCCESS == rc))
        {
            ...
            rowId++;
                ...
                if (rowId >= Relay_Cal_Bytes_ROWS_IN_ARRAY)
                {
                    arrayId++;
                    rowId = 0u;
                }
